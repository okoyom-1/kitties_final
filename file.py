...
def train_step(model: torch.nn.Module, 
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        # 2. Посчитать и собрать ошибки при обучении
        loss = loss_fn(y_pred, y)
        train_loss += loss.item() 

        # 3. Оптимизатор делает шаг
        optimizer.zero_grad()

        # 4. Происходит обратный проход
        loss.backward()

        # 5. Шаг оптимизатора
        optimizer.step()

        # Вычислить точность и добавить в сумму точностей
        y_pred_class = torch.argmax(torch.softmax(y_pred, dim=1), dim=1)
        train_acc += (y_pred_class == y).sum().item()/len(y_pred)

    # Получение среднего значения ошибки и точности
